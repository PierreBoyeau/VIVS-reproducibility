...
def train_scvi(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if hasnt_improved > patience:
            break
    best_x_params = state.params
    best_x_batch_stats = batch_stats
...
def train_statistic(
    ...
    def train_step(state, batch_stats, inputs, dropout_rng):
        def loss_fn(params, batch_stats):
            ...
            x_ = (
                jnp.log1p(log1p_factor * x / jnp.sum(x, axis=-1, keepdims=True))
                if normalize_x
                else x
            )
            # x_ = x
    ...
    def evaluate(state, batch_stats, inputs):
        ...
        loss = _losses["loss"]
        all_loss = _losses["all_loss"]
        return loss, all_loss.mean(0)
    ...
    for epoch in tqdm(range(n_epochs)):
        for batch in train_dl:
            X = batch[REGISTRY_KEYS.X_KEY]
            protein_exp = batch["protein_expression"]
        ...
        val_loss = 0
        _val_losses = jnp.zeros(n_proteins)
        n_batches = 0
        for batch in val_dl:
            X = batch[REGISTRY_KEYS.X_KEY]
            protein_exp = batch["protein_expression"]
            ...
            validation_loss, all_losses = evaluate(
                state, batch_stats, [X, sample_batch, protein_exp]
            )
            n_batches += 1
            val_loss += validation_loss
            _val_losses += all_losses / n_batches
        val_losses.append(np.array(jax.device_get(_val_losses))[None])
    ...
    overall_val_loss = val_losses.mean(1)
    overall_val_loss = pd.DataFrame(
        dict(
            iterate=np.arange(overall_val_loss.shape[0]),
            metric="stat_val_loss",
            value=overall_val_loss,
        )
    )
    ...
    val_losses = (
        pd.DataFrame(
            val_losses,
            index=np.arange(val_losses.shape[0]),
            columns=["stat_val_loss_" + str(i) for i in range(n_proteins)],
        )
        .stack()
        .to_frame("value")
        .reset_index()
        .rename(columns={"level_0": "iterate", "level_1": "metric"})
    )
    val_losses = pd.concat([val_losses, overall_val_loss])
    # losses = pd.DataFrame(losses).append(val_losses)
    return val_losses, best_params, best_batch_stats
